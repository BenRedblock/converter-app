...
import { AudioConvertHistoryType, AudioConvertOptions } from 'renderer/utils/types';
...
command.addOption("-qscale 1")
  ...
  command.on("progress",async (progress)=> {
    mainWindow?.webContents.send("audio-convert", await calculateProgress(options.inputPath, progress, seconds))
  })
  .on("end",()=> {
    mainWindow?.webContents.send("audio-convert", [true, 'Finished converting'])
    const history: AudioConvertHistoryType = {inputAudio: options.inputPath, format: options.format, outputAudio: destinationpath, timestamp: new Date()}
    mainWindow?.webContents.send("history", "AudioConvert", history)
    clearInterval(intervalId)
  })
  .on("error",(error)=> {
    console.log(error.message)
    mainWindow?.webContents.send("audio-convert", [false, `Failed with Error: ${error.message}`])
    clearInterval(intervalId)
  })
