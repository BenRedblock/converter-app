...
export default function videoConvert(options: VideoConvertOptions) {
  ...
  command
    ...
    .on('end', () => {
      mainWindow?.webContents.send('video-convert', [
        true,
        'Finished converting',
      ]);
      const history: VideoConvertHistoryType = {
        inputVideo: options.inputPath,
        format: options.format,
        outputVideo: destinationpath,
        timestamp: new Date(),
      };
